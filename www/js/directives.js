			...
			link: function (scope, element, attrs, ngModel) {
				...
				popupPromise.then(function (el) {
					...
					scope.selectLocation = function (location) {
						...
						setLocationController(location);
					...
					scope.setCurrentLocation = function () {
						...
						el.element.css('display', 'none');
						$ionicBackdrop.release();
						getLocation().then(reverseGeocoding).then(function (location) {
							ngModel.$setViewValue(location);
				...
				function setLocationController(location) {
					ngModel.$setViewValue(location);
					element.attr('value', location.formatted_address);
					ngModel.$render();
				...
				if (scope.currentLocation) {
					$timeout(getLocation().then(reverseGeocoding).then(setLocationController));
				}
			}
		}
	});